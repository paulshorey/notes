  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch("/api/embeddings/debug", {
        ...
        body: JSON.stringify({ search, description, searchTask, passageTask }),
      })
        ...
        <div className={styles.leftCol}>
          <div className={styles.inputGrid}>
            ...
            <Text variant="caption-2" color="secondary">
              Jina <code>task</code> param sent with each embedding request. Defaults match the app: <code>retrieval.query</code> for search, <code>retrieval.passage</code> for stored notes.
            </Text>
          ...
          {result && (
            <div className={styles.results}>
              <div className={styles.scoringCard}>
                <Text variant="header-2">Search score</Text>
                <Text variant="body-2" color="secondary">
                  Formula: <code>{result.scoring.formula}</code>
                </Text>

                <div className={styles.scoreRow}>
                  <Text variant="header-1">Note similarity</Text>
                  <span className={styles.scoreValue} style={{ color: simColor(result.scoring.score) }}>
                    {formatPct(result.scoring.score)}
                  </span>
                </div>
              </div>
            </div>
