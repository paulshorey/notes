...
- `validate-notes-contract.mjs` - strict contract checker for `Models.kt`, `JsonCodec.kt`, and `NotesApiClient.kt` against generated `@lib/db-notes` contract JSON.
...
- `validate-notes-contract.mjs` checks field order and even some string snippets, so cosmetic Kotlin refactors can break it.
