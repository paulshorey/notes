...
if (getScalar(apiTokenUserFkResult.rows, "count") !== 1) {
  throw new Error(
    "Missing expected foreign key constraint: user_api_token_v1_user_id_fkey"
  );
}
...
if (getScalar(apiTokenHashUniqueResult.rows, "count") !== 1) {
  throw new Error(
    "Missing expected unique constraint: user_api_token_v1_token_hash_key"
  );
}
...
const noteWorkflowStatusColumnResult = await client.query(`
  SELECT is_nullable
  FROM information_schema.columns
  WHERE table_schema = 'public'
    AND table_name = 'user_note_v1'
...
if (!noteWorkflowStatusColumnResult.rows[0]) {
  throw new Error("Missing expected column on user_note_v1: workflow_status_id");
}

if (getScalar(noteWorkflowStatusColumnResult.rows, "is_nullable") !== "YES") {
  throw new Error("Expected public.user_note_v1.workflow_status_id to be nullable");
}

const noteTimeCompletedColumnResult = await client.query(`
  SELECT is_nullable
  FROM information_schema.columns
  WHERE table_schema = 'public'
    AND table_name = 'user_note_v1'
    AND column_name = 'time_completed'
`);

if (!noteTimeCompletedColumnResult.rows[0]) {
  throw new Error("Missing expected column on user_note_v1: time_completed");
}

if (getScalar(noteTimeCompletedColumnResult.rows, "is_nullable") !== "YES") {
  throw new Error("Expected public.user_note_v1.time_completed to be nullable");
}

const noteWorkflowStatusFkResult = await client.query(`
  SELECT COUNT(*)::int AS count
  FROM pg_constraint
  WHERE conname = 'user_note_v1_workflow_status_id_fkey'
`);

if (getScalar(noteWorkflowStatusFkResult.rows, "count") !== 1) {
  throw new Error(
    "Missing expected foreign key constraint: user_note_v1_workflow_status_id_fkey"
  );
}

const workflowStatusUserFkResult = await client.query(`
  SELECT COUNT(*)::int AS count
  FROM pg_constraint
  WHERE conname = 'user_workflow_status_v1_user_id_fkey'
`);

if (getScalar(workflowStatusUserFkResult.rows, "count") !== 1) {
  throw new Error(
    "Missing expected foreign key constraint: user_workflow_status_v1_user_id_fkey"
  );
}

const workflowStatusUniqueResult = await client.query(`
  SELECT COUNT(*)::int AS count
  FROM pg_constraint
  WHERE conname = 'user_workflow_status_v1_user_id_label_key'
`);

if (getScalar(workflowStatusUniqueResult.rows, "count") !== 1) {
  throw new Error(
    "Missing expected unique constraint: user_workflow_status_v1_user_id_label_key"
  );
}

const workflowStatusLowercaseCheckResult = await client.query(`
  SELECT COUNT(*)::int AS count
  FROM pg_constraint
  WHERE conname = 'user_workflow_status_v1_label_lowercase_check'
`);

if (getScalar(workflowStatusLowercaseCheckResult.rows, "count") !== 1) {
  throw new Error(
    "Missing expected check constraint: user_workflow_status_v1_label_lowercase_check"
  );
}

const workflowStatusUserIndexResult = await client.query(`
  SELECT COUNT(*)::int AS count
  FROM pg_indexes
  WHERE schemaname = 'public'
    AND indexname = 'user_workflow_status_v1_user_id_idx'
`);

if (getScalar(workflowStatusUserIndexResult.rows, "count") !== 1) {
  throw new Error("Missing expected index: user_workflow_status_v1_user_id_idx");
}

const noteWorkflowStatusIndexResult = await client.query(`
  SELECT COUNT(*)::int AS count
  FROM pg_indexes
  WHERE schemaname = 'public'
    AND indexname = 'user_note_v1_workflow_status_id_idx'
`);

if (getScalar(noteWorkflowStatusIndexResult.rows, "count") !== 1) {
  throw new Error("Missing expected index: user_note_v1_workflow_status_id_idx");
}
...
const rowTimestampTriggersResult = await client.query(`
  ...
  WHERE NOT tgisinternal
    AND tgname IN (
      ...
      'user_workflow_status_v1_apply_row_timestamps_v1'
    )
`);

if (getScalar(rowTimestampTriggersResult.rows, "count") !== 5) {
