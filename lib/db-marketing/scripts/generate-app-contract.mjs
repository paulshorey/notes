...
const parseInterfaceBody = (sourceFile, body) =>
  body
    .split("\n")
    .map((line) => line.trim())
    .filter(Boolean)
...
const parseInterfaces = async () => {
  ...
  for (const sourceFile of sourceFiles) {
    ...
    for (const match of matches) {
      const [, modelName, parentName, body] = match;
      rawInterfaces.set(modelName, {
        parentName: parentName ?? null,
        fields: parseInterfaceBody(sourceFile, body),
      });
    }
  }

  const resolveFields = (modelName, visited = new Set()) => {
    if (visited.has(modelName)) {
      throw new Error(`Circular interface inheritance detected at ${modelName}`);
    }

    visited.add(modelName);
    const raw = rawInterfaces.get(modelName);

    if (!raw) {
      throw new Error(`Missing interface definition for ${modelName}`);
    }

    if (raw.parentName === null) {
      return raw.fields;
    }

    return [...resolveFields(raw.parentName, visited), ...raw.fields];
  };

  for (const modelName of rawInterfaces.keys()) {
    models[modelName] = { fields: resolveFields(modelName) };
  }
...
if (existing !== generated) {
  throw new Error(
    "generated/contracts/notes-app.json is out of date. Run: pnpm --filter @lib/db-marketing app:contract:generate",
  );
}
