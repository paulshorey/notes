...
const typesPath = path.join(outDir, "db-types.ts")
await fs.mkdir(outDir, { recursive: true })

// db:verify diffs this file against git, so it has to come out byte-identical
// to what `pnpm format` would produce.
const prettierConfig = await prettier.resolveConfig(typesPath)
const formatted = await prettier.format(output, { ...prettierConfig, filepath: typesPath })
await fs.writeFile(typesPath, formatted, "utf8")
