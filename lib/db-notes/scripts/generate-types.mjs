...
// Keep this file's output compatible with repo Prettier (`semi: false`,
// quoteProps as-needed). db:verify diffs the generated file; `pnpm format`
// would otherwise rewrite it and fail the contract check.
const isIdentifier = (value) => /^[A-Za-z_$][A-Za-z0-9_$]*$/.test(value)
const formatProp = (name) => (isIdentifier(name) ? name : JSON.stringify(name))
...
const baseTypeFromRow = (row) => {
  ...
  const mapSimple = (t) => {
    if (["int2", "int4", "int8", "float4", "float8", "numeric", "money"].includes(t))
      return "number"
    if (["bool"].includes(t)) return "boolean"
    if (["json", "jsonb"].includes(t)) return "unknown"
    if (["date", "timestamp", "timestamptz", "time", "timetz"].includes(t)) return "Date"
    if (["uuid", "text", "varchar", "bpchar", "char", "name", "vector"].includes(t)) return "string"
...
output += "// Run: pnpm --filter @lib/db-notes db:types:generate\n\n"
...
for (const [tableName, columns] of tables.entries()) {
  ...
  for (const column of columns) {
    ...
    output += `  ${formatProp(column.column_name)}: ${baseType}${nullable}\n`
  }
  output += "}\n\n"
}
...
for (const tableName of tables.keys()) {
  const iface = `${pascalCase(tableName)}Row`
  output += `  ${formatProp(tableName)}: ${iface}\n`
}
output += "}\n"
