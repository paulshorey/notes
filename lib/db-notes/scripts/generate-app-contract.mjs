...
const parseInterfaces = async () => {
  ...
  for (const sourceFile of sourceFiles) {
    ...
    const matches = content.matchAll(/export interface\s+([A-Za-z0-9_]+)\s*\{([\s\S]*?)\n\}/g)
    ...
    for (const match of matches) {
      ...
      const fields = body
        ...
        .map((line) => line.trim())
        .filter(Boolean)
        // Doc comments describe fields for humans and carry no contract data.
...
if (existing !== generated) {
  throw new Error(
    "generated/contracts/notes-app.json is out of date. Run: pnpm --filter @lib/db-notes app:contract:generate",
  )
}
