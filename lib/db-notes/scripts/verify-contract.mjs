...
const droppedCategoryTableResult = await client.query(`
  SELECT COUNT(*)::int AS count
  FROM information_schema.tables
  WHERE table_schema = 'public'
...
if (getScalar(droppedCategoryTableResult.rows, "count") !== 0) {
  throw new Error(
    "Expected public.user_note_category_v1 to be removed (dropped by 202609060100)",
  )
}

const droppedCategoryConstraintsResult = await client.query(`
  SELECT COUNT(*)::int AS count
  FROM pg_constraint
  WHERE conname IN (
    'user_note_v1_category_id_fkey',
    'user_note_category_v1_user_id_fkey',
    'user_note_category_v1_user_id_label_key',
    'user_note_category_v1_label_lowercase_check'
  )
`)

if (getScalar(droppedCategoryConstraintsResult.rows, "count") !== 0) {
  throw new Error(
    "Expected leftover category constraints to be removed (dropped by 202609060100)",
  )
}
...
const droppedNoteCategoryColumnResult = await client.query(`
  SELECT COUNT(*)::int AS count
  FROM information_schema.columns
  WHERE table_schema = 'public'
    AND table_name = 'user_note_v1'
...
if (getScalar(droppedNoteCategoryColumnResult.rows, "count") !== 0) {
  throw new Error(
    "Expected public.user_note_v1.category_id to be removed (dropped by 202609060100)",
  )
}
...
const droppedCategoryIndexesResult = await client.query(`
  SELECT COUNT(*)::int AS count
  FROM pg_indexes
  WHERE schemaname = 'public'
    AND indexname IN (
      'user_note_category_v1_user_id_idx',
      'user_note_v1_category_id_idx',
      'user_note_category_v1_category_embedding_hnsw_idx'
    )
`)

if (getScalar(droppedCategoryIndexesResult.rows, "count") !== 0) {
  throw new Error(
    "Expected leftover category indexes to be removed (dropped by 202609060100)",
  )
}
...
if (getScalar(rowTimestampTriggersResult.rows, "count") !== 3) {
