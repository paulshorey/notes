/**
 * Standalone script to regenerate ALL embeddings for notes, categories, and tags.
 *
 * Usage:
 *   DB_NOTES_URL=… JINA_API_KEY=… node scripts/regenerate-embeddings.mjs [--user <id>] [--dry-run] [--batch-size <n>]
 *
 * Flags:
 ...
 * in sync with lib/db-notes/services/notes-embeddings.ts — that file is
...
if (!connectionString) {
  throw new Error("DB_NOTES_URL environment variable is required.")
}
...
async function fetchEmbeddings(inputs, task) {
  ...
  if (!response.ok) {
    throw new Error(`Jina error (${response.status}): ${payload?.detail ?? "unknown"}`)
  }
...
async function getUserIds() {
  ...
  const { rows } = await client.query("SELECT id FROM public.user_v1 ORDER BY id")
...
async function getTagsForUser(userId) {
  const { rows } = await client.query(
    `SELECT t.id, t.label
     FROM public.workspace_note_tag_v1 t
     JOIN public.user_workspace_v1 w ON w.id = t.workspace_id
     WHERE w.user_id = $1
       AND NULLIF(btrim(t.label), '') IS NOT NULL
     ORDER BY t.id`,
    [userId],
  )
  return rows
...
async function getCategoriesForUser(userId) {
  const { rows } = await client.query(
    `SELECT c.id, c.label
     ...
     JOIN public.user_workspace_v1 w ON w.id = c.workspace_id
     WHERE w.user_id = $1
  ...
  for (let i = 0; i < tags.length; i += BATCH_SIZE) {
    ...
    for (let j = 0; j < nonEmpty.length; j++) {
      ...
      if (DRY_RUN) {
        ...
        await client.query(
          `UPDATE public.workspace_note_tag_v1 t
...
async function regenerateCategories(userId, categories) {
  ...
  for (let i = 0; i < categories.length; i += BATCH_SIZE) {
    ...
    const nonEmpty = batch
      .map((category) => ({ ...category, text: normalizeText(category.label) }))
      .filter((category) => category.text !== "")

    if (nonEmpty.length === 0) continue

    const embeddings = await fetchEmbeddings(
      nonEmpty.map((category) => category.text),
      "retrieval.passage",
    )

    for (let j = 0; j < nonEmpty.length; j++) {
      const category = nonEmpty[j]
      const vectorLiteral = JSON.stringify(embeddings[j])

      if (DRY_RUN) {
        console.log(`  [dry-run] category ${category.id} "${category.label}" → embedding`)
      } else {
        await client.query(
          `UPDATE public.workspace_note_category_v1 c
           SET category_embedding = $1::vector,
               embedding_model = $2,
               embedding_updated_at = $3
           FROM public.user_workspace_v1 w
           WHERE c.id = $4
             ...
             AND w.user_id = $5`,
          [vectorLiteral, CURRENT_NOTE_EMBEDDING_MODEL, now, category.id, userId],
        )
      }
...
for (const userId of userIds) {
  ...
  console.log(
    `User ${userId}: ${categories.length} categories, ${tags.length} tags, ${notes.length} notes`,
  )

  const categoryUpdated = await regenerateCategories(userId, categories)
  totalCategories += categoryUpdated

  const tagUpdated = await regenerateTags(userId, tags)
  totalTags += tagUpdated

  const noteUpdated = await regenerateNotes(userId, notes)
  totalNotes += noteUpdated

  console.log(`  → ${categoryUpdated} categories, ${tagUpdated} tags, ${noteUpdated} notes updated`)
}

console.log(
  `\nDone. ${totalCategories} categories, ${totalTags} tags, ${totalNotes} notes ${DRY_RUN ? "would be " : ""}updated across ${userIds.length} user(s).`,
)
