...
- Tests: `pnpm --filter @lib/db-notes test` (node test runner via tsx).
  ...
  — never to `DB_NOTES_URL`. In Cursor Cloud, start the local Postgres 17
  cluster (see the workspace `AGENTS.md` Cursor Cloud section) and point
  `DB_NOTES_TEST_URL` at `postgres:///notes_test?host=/var/run/postgresql`.
  CI's verify-notes job runs it against its throwaway migrated container.
