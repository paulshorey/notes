 ...
 *
 * Usage:
 *   node scripts/fetch-infisical.mjs /notes/apps/notes-next apps/notes-next/.env
...
const APPS = [
  {
    ...
    infisicalPaths: ["/notes/apps/notes-next"],
  },
]
...
async function fetchSecrets(infisicalPath, outputFile, options) {
  ...
  const returnedPaths = [
    ...new Set(secrets.secrets.map((secret) => normalizeSecretPath(secret.secretPath))),
  ]

  if (requestedPath !== "/" && returnedPaths.length > 0 && !returnedPaths.includes(requestedPath)) {
    throw new Error(
      `Infisical returned secrets from ${returnedPaths.join(", ")} for requested path ${requestedPath}; refusing to overwrite ${outputFile}.`,
    )
  }
...
async function main() {
  ...
  if (!clientId) {
    ...
    console.error(
      "Important: this must be the machine identity Client ID, not the identity record ID.",
    )
    process.exit(1)
  }
  ...
  if (!projectId) {
    ...
    console.error(
      "Find it in Project Settings or in the URL: https://app.infisical.com/project/{projectId}/...",
    )
    process.exit(1)
  }
  ...
  if (args.includes("--all") || args.length === 0) {
    ...
    if (!infisicalPath || !outputFile) {
      ...
      console.error("Example: fetch-infisical.mjs /notes/apps/notes-next apps/notes-next/.env")
      process.exit(1)
    }
