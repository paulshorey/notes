...
# Dump data only (no DDL) for selected public tables into a local SQL file.
...
#   ./scripts/db/notes-backup-data.sh [OUTFILE.sql]
#   ./scripts/db/notes-backup-data.sh -t other_table [OUTFILE.sql]
#
# With no OUTFILE, writes to scripts/db/backups/notes-data-YYYYMMDD-HHMMSS.sql

usage() {
  cat >&2 <<'EOF'
Dump data only (no DDL) for selected public tables. Requires DB_NOTES_URL.

Usage: notes-backup-data.sh [-t TABLE]... [OUTFILE.sql]
  -t, --table TABLE   Public table name (repeatable). Default: NOTES_DB_DEFAULT_TABLES
                      in common.sh (user_v1, user_note_v1, user_taxonomy_v1,
                      user_taxonomy_level_v1, user_note_tag_v1, ...)
...
if [[ -z "$outfile" ]]; then
  ...
  mkdir -p "$backup_dir"
  outfile="${backup_dir}/notes-data-$(date +%Y%m%d-%H%M%S).sql"
fi
...
echo "Wrote data-only dump for tables: ${tables[*]}" >&2
echo "$outfile"
